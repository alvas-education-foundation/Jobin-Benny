...
$amt=$_GET['amt'];
include('../connect.php');
$sql ="SELECT * FROM sales WHERE date='".$invoice."' and amount ='".$amt."'";
$result=$db->query($sql);
...
<div style="margin: 0 auto; padding: 20px; width: 900px; font-weight: normal;">
	...
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		...
		<tbody>
				...
				<?php
					$id=$_GET['invoice'];
					$sql ="SELECT * FROM sales_order WHERE date= '".$id."'";
					$result=$db->query($sql);
					for($i=0; $row = $result->fetch_assoc(); $i++){
				?>
				<tr class="record">
				...
				<tr>
					...
					$sdsd=$_GET['invoice'];
					$sdsd1=$_GET['amt'];
					$sql = "SELECT sum(amount) FROM sales_order WHERE date= '".$sdsd."' and amount = '".$sdsd1."'";
					$resultas=$db->query($sql);
					for($i=0; $rowas = $resultas->fetch_assoc(); $i++){
					$fgfg=$rowas['sum(amount)'];
					echo $sdsd1 ;//formatMoney($fgfg, true);
					}
					?>
					</strong></td>
				</tr>
				...
				<tr>
					...
					?>
                                                                                       ...
                                                                                       <center>   <b>  "THANK YOU"     <br>  VISIT AGAIN </b></center>
	<?php 
