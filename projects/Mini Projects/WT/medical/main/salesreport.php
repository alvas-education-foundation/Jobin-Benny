...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$sql="SELECT * FROM sales WHERE date BETWEEN '".$d1."' AND '".$d2."' ORDER by transaction_id DESC ";
				$result = $db->query($sql);
				// echo $db->error;
				// $result->bind_param(':a', $d1);
				// $result->bind_param(':b', $d2);
				//$result->execute();
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$sql="SELECT sum(amount) FROM sales WHERE date BETWEEN '".$d1."' AND '".$d2."'";
				$result = $db->query($sql);
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php 
				$sql= "SELECT sum(profit) FROM sales WHERE date BETWEEN '".$d1."' AND '".$d2."'";
				$resultia = $db->query($sql);
				// $resultia->bindParam(':c', $d1);
				// $resultia->bindParam(':d', $d2);
				//$resultia->execute();
				for($i=0; $cxz = $resultia->fetch_assoc(); $i++){
				$zxc=$cxz['sum(profit)'];
				echo formatMoney($zxc, true);
				}
				?>
