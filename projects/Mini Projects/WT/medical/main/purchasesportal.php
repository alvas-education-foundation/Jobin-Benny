...
$id=$_GET['iv'];
include('../connect.php');
$sql = "SELECT * FROM purchases WHERE invoice_number= $id";
$resultaz=$db->query($sql);
for($i=0; $rowaz = $resultaz->fetch_assoc(); $i++){
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				$id=$_GET['iv'];
				$sql = "SELECT * FROM purchases_item WHERE invoice= $id";
				$result=$db->query($sql);
				for($i=0; $row = $result->fetch_assoc(); $i++){
			?>
			<tr class="record">
			<td><?php
			$rrrrrrr=$row['name'];
			$sql ="SELECT * FROM products WHERE product_code= $rrrrrrr";
			...
			<tr>
				...
				$sdsd=$_GET['iv'];
				$sql ="SELECT sum(cost) FROM purchases_item WHERE invoice= $sdsd";
				$resultas=$db->query($sql);
