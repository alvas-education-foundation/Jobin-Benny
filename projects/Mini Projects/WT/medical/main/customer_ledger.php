...
include('../connect.php');
$tftft=$_GET['cname'];
$sql ="SELECT * FROM sales WHERE invoice_number= $tftft";
$resulta=$db->query($sql);
for($i=0; $rowa = $resulta->fetch_accoc(); $i++){
$name=$rowa['name'];
$amount=$rowa['amount'];
}
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				$tftft=$_GET['cname'];
				$sql ="SELECT * FROM collection WHERE name= $tftft ORDER BY transaction_id ASC";
				$result=$db->query($sql);
